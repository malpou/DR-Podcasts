    ...
    public async Task<IEnumerable<Episode>?> Handle(GetEpisodesQuery request, CancellationToken cancellationToken)
    {
        ...
        if (fromDate.HasValue && toDate.HasValue)
        {
            if (string.IsNullOrWhiteSpace(podcastName))
                ...
                logger.LogInformation("Getting episodes for podcast {PodcastName} from {FromDate} to {ToDate}",
                    podcastName, fromDate, toDate);
        ...
        if (pageNumber.HasValue && pageSize.HasValue)
        {
            if (string.IsNullOrWhiteSpace(podcastName))
                ...
                logger.LogInformation(
                    "Getting episodes page {PageNumber} with size {PageSize} for podcast {PodcastName}", pageNumber,
                    pageSize, podcastName);
